...
const VideoCard = ({ item }) => {
  ...
  return (
    <View style={styles.card}>
      ...
      {/* <Video
        source={{ uri: `http://192.168.0.104:5000/reels${item.filepath}?token` }}
        style={styles.image}
        isLooping={false}
      ...
      <Image
        ...
        resizeMode={ResizeMode.COVER}
        source={{ uri: `http://192.168.0.104:5000${item.filepath}-thumbnail.png?token` }} style={styles.image} />
      {/* </TouchableOpacity> */}
    </View>
  );
};
...
const VideoList = (props) => {
  ...
  const fetchReel = async (userID = null) => {
    var API = userID ? `http://192.168.0.104:5000/api/v1/reels/by-userID/${userID}` : "http://192.168.0.104:5000/api/v1/reels/get-latest"
    axios.get(API)
      .then((response) => {
