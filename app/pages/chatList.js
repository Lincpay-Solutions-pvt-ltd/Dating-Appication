...
const ChatList = () => {
  ...
  const { id, name, profilePic, gender, isMessage, userID } = useLocalSearchParams();
  ...
  const fetchFollowingData = async (currentUser) => {
    try {
      const response = await axios.get(
        `${process.env.EXPO_PUBLIC_API_BASE_URL}/api/v1/follow/getFollowingList/${userID || currentUser.userID}`
      );
