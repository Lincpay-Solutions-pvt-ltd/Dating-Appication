...
import { View, Button, StyleSheet, Text, Alert, Platform, PermissionsAndroid, TouchableOpacity } from 'react-native';
...
const WebRTCComponent = () => {
  ...
  const { from_, to_, chatID, isCaller, status_, currentUser_ } = useLocalSearchParams();
  ...
  const from = from_;
  const to = to_;
  const currentUser = JSON.parse(currentUser_);
  console.log("✅ ✅ to", to);
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      console.log("isCaller", isCaller, "isConnected", isConnected, "status", status);

      if (isCaller && !isConnected && status !== 'connected') {
  ...
  return (
    <View style={styles.container}>
      ...
      {/* Remote Video */}
      {remoteStreamURL ? (
        <RTCView style={styles.remoteVideo} streamURL={remoteStreamURL} mirror={false} zOrder={1} objectFit="cover" />
      ) : (
      ...
      {/* Local Video */}
      {localStreamURL && (
        <RTCView style={styles.localVideo} streamURL={localStreamURL} objectFit="cover" mirror={true} zOrder={2} />
      )}
      ...
      <View style={styles.controlsContainer}>
        ...
        <TouchableOpacity style={styles.controlButton} onPress={toggleVideo}>
          <Ionicons name={isVideoPaused ? 'videocam-off' : 'videocam'} size={28} color="#fff" />
        </TouchableOpacity>
        {/* End Call */}
